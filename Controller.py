...
class Controller:
    ...
    def btn_send_click(self):
        text = self.__view.char_input.get().upper()
        self.__model.check_user_input(text)
        self.__view.char_input.delete(0, 'end')
        self.__view.lbl_result['text'] = " ".join(self.__model.correct_letters).upper()
        missing_letters = ','.join(map(str.upper, self.__model.wrong_letters))
        self.__view.lbl_error['text'] = f'Vigased tähed {missing_letters}'
